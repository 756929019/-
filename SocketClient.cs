...
namespace Water.Utilites
{
    ...
    public class SocketClient
    {
        ...
        private SocketClient()
        {
            try
            {
                con_ip = System.Configuration.ConfigurationManager.AppSettings["Socket_Server_IP"].ToString();
                con_port = Int32.Parse(System.Configuration.ConfigurationManager.AppSettings["Socket_Server_Port"].ToString());
                con_msg_length = Int32.Parse(System.Configuration.ConfigurationManager.AppSettings["Socket_Msg_Length"].ToString());
                start_msg = "{UserNo:'" + Employee.Current.EmployeeID + "',UserName:'" + Employee.Current.Name + "',UnitNo:'" + Employee.Current.DepartmentID + "',UnitName:'" + Employee.Current.DepartmentName + "'}";
            }
            catch(Exception ex)
            {
        ...
        public void BeginConnect()
        {
            try
            {
                ...
                IPAddress ip = IPAddress.Parse(con_ip);

                clientSocket.BeginConnect(ip, con_port, (args) =>
                {
                    if (args.IsCompleted)   //判断该异步操作是否执行完毕
                    {
                        Byte[] bytesSend = new Byte[con_msg_length];
        ...
        private void recMsg()
        {
            while (true) //持续监听服务端发来的消息
            {
                ...
                byte[] arrRecMsg = new byte[con_msg_length];
                int length = 0;
                try
                {
                ...
                catch(Exception ex)
                {
                    ...
                    {
                        clientSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                        clientSocket.BeginConnect(IPAddress.Parse(con_ip), con_port, (args) =>
                        {
                            if (args.IsCompleted)   //判断该异步操作是否执行完毕
                            {
                                Byte[] bytesSend = new Byte[con_msg_length];
        ...
        public void SendMsg(string title, string content, string userno, string unitno, string menu = "")
        {
            try
            {
                string msg = "{title:'" + title + "',content:'" + content + "',menu:'" + menu + "'}";
                //userno:"001,002,003"
                ...
                string msgcon = "{msgType:'msg',msgContent:'" + msg + "',UserNo:'" + userno + "',UnitNo:'" + unitno + "'}";
